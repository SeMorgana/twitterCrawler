/**
 * Collect 160(this number can be changed based on needs) followers of a user randomly.
...
public class GetFollowers{
    ...
    /*
     *	If there are fewer than 160, then return them all.
     * Otherwise return 160 from the original list
     */
    public static List<Long> getRandom160(List<Long> al_follower_ids){
        if(al_follower_ids.size() <= 160){
            return al_follower_ids;
        } else {
            List<Long> al_random_160 = new ArrayList<Long>();
    ...
    public static boolean writeResponseTo(String user_id_str, List<Long> al_follower_ids){
        try{
            ...
            List<Long> al_random_160 = getRandom160(al_follower_ids);

            for(int i=0;i<al_random_160.size();i++){
                String query = "INSERT DELAYED "+table_name_fr_fo +" values(\'" + user_id_str+ "\'," + "\'"+ String.valueOf(al_random_160.get(i))+ "\');";
